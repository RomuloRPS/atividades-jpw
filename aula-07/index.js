...
app.get('/', (req, res) => {
    ...
    const search = queryString.nome;

    if (search) {
        const house = data.find(element => element.members.find(element2 => element2.name.toUpperCase().match(searchName.toUpperCase())));

        return res.send(house ? house.name : 'Não encontrado');
    }
